...
MAIN_CPPS 		:= app_shell.c sh.c user_lib.c
MAIN_OBJS			:= $(addprefix $(OBJ_DIR)/user/, $(patsubst %.c,%.o,$(MAIN_CPPS)))
MAIN_TARGET		:= $(OBJ_DIR)/app_shell

ECHO_CPPS			:= echo.c user_lib.c
ECHO_OBJS			:= $(addprefix $(OBJ_DIR)/user/, $(patsubst %.c,%.o,$(ECHO_CPPS)))
ECHO_TARGET		:= $(OBJ_DIR)/echo

CAT_CPPS			:= cat.c user_lib.c
CAT_OBJS			:= $(addprefix $(OBJ_DIR)/user/, $(patsubst %.c,%.o,$(CAT_CPPS)))
CAT_TARGET		:= $(OBJ_DIR)/cat

TOP_CPPS			:= top.c user_lib.c
TOP_OBJS			:= $(addprefix $(OBJ_DIR)/user/, $(patsubst %.c,%.o,$(TOP_CPPS)))
TOP_TARGET		:= $(OBJ_DIR)/top

CREATEP_CPPS		:= createproc.c user_lib.c
CREATEP_OBJS		:= $(addprefix $(OBJ_DIR)/user/, $(patsubst %.c,%.o,$(CREATEP_CPPS)))
CREATEP_TARGET	:= $(OBJ_DIR)/createproc

TOUCH_CPPS		:= touch.c user_lib.c
TOUCH_OBJS		:= $(addprefix $(OBJ_DIR)/user/, $(patsubst %.c,%.o,$(TOUCH_CPPS)))
TOUCH_TARGET	:= $(OBJ_DIR)/touch
...
USER_TARGET		:= $(addprefix $(OBJ_DIR)/, $(USER_TARGET))
...
$(MAIN_TARGET): $(OBJ_DIR) $(UTIL_LIB) $(USER_OBJS)
	@echo "linking" $@	...	
	@$(COMPILE) --entry=main $(MAIN_OBJS) $(UTIL_LIB) -o $@
	@echo "User app has been built into" \"$@\"

$(ECHO_TARGET): $(OBJ_DIR) $(UTIL_LIB) $(USER_OBJS)
	@echo "linking" $@	...	
	@$(COMPILE) --entry=main $(ECHO_OBJS) $(UTIL_LIB) -o $@
	@echo "User app has been built into" \"$@\"

$(CAT_TARGET): $(OBJ_DIR) $(UTIL_LIB) $(USER_OBJS)
	@echo "linking" $@	...	
	@$(COMPILE) --entry=main $(CAT_OBJS) $(UTIL_LIB) -o $@
	@echo "User app has been built into" \"$@\"

$(TOP_TARGET): $(OBJ_DIR) $(UTIL_LIB) $(USER_OBJS)
	@echo "linking" $@	...	
	@$(COMPILE) --entry=main $(TOP_OBJS) $(UTIL_LIB) -o $@
	@echo "User app has been built into" \"$@\"

$(CREATEP_TARGET): $(OBJ_DIR) $(UTIL_LIB) $(USER_OBJS)
	@echo "linking" $@	...	
	@$(COMPILE) --entry=main $(CREATEP_OBJS) $(UTIL_LIB) -o $@
	@echo "User app has been built into" \"$@\"

$(TOUCH_TARGET): $(OBJ_DIR) $(UTIL_LIB) $(USER_OBJS)
	@echo "linking" $@	...	
	@$(COMPILE) --entry=main $(TOUCH_OBJS) $(UTIL_LIB) -o $@
	@echo "User app has been built into" \"$@\"
